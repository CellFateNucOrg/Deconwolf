...
Once you have the psfs you need, you can deconvolve images with the `dw_apptainer.sh`script. Adjust the parameters according to the microscope and image settings your images have. See detailed explanation in [section 3.2](#32-deconvolve-images) below. Once you have modified the parameters in `dw_apptainer.sh` you can then run the script with:
...
- `tiles`: tile size in pixels. If you process large files and run into memory issues, tiling may help. Set to 0 to run the deconvolution without tiling.
