...
- `scale`: whether to rescale the deconvolved images. If True (default), each pixel value is multiplied by $\frac{2^{16}-1}{I_{max}}$, i.e., the bit depth divided by the maximum pixel value. Rescaling prevents clipping (image saturation) but makes the histograms less comparable between images.
